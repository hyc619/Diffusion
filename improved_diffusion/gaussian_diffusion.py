...
class GaussianDiffusion:
    ...
    #数据的真实分布，根据x_0和t可以计算出x_t时刻的均值方差，对应公式8
    def q_mean_variance(self, x_start, t):
        ...
        """
        mean = (
            _extract_into_tensor(self.sqrt_alphas_cumprod, t, x_start.shape) * x_start # 取出张量self.sqrt_alphas_cumprod的第t个，和x[0]的shape相同
        )
    ...
    #对上面的函数进行采样，重参数化的过程，给定x0和t，可以直接采样出xt
    ...
    #基于x_0,x_t,t计算出后验真实分布的均值和方差,公式10
    ...
    #可以得到前一时刻的均值和方差，得到逆扩散过程的均值和方差，也包括对初始数据分布x_0的预测
    def p_mean_variance2(
        ...
        if 'debug_x_t' in model_kwargs:
            ...
            flag=False
        print(model_kwargs)
        model_output = model(x, self._scale_timesteps(t), **model_kwargs)   #给定当前时刻的x_t和t，可以得到x_{t-1}的均值和方差
        ...
        if self.model_var_type in [ModelVarType.LEARNED, ModelVarType.LEARNED_RANGE]:
            ...
            #预测方差插值的系数，预测的范围是在[-1,1]之间,公式(14)
            else:
                ...
                frac = (model_var_values + 1) / 2  #从[-1,1]转化成[0,1]之间
        ...
        #这个网络可以预测噪声，可以预期期望均值，也可以直接预测x_0
    ...
    #通过预测出来的噪声eps,x[t]来预测x[0],对应公式12
    ...
    def ddim_sample_loop_progressive_prompt(
        self,
        model,
        shape,
        noise=None,
        ...
        for i in indices:
            t = th.tensor([i] * 60, device=device)
            with th.no_grad():
                out = self.ddim_sample(
                    model,
                    img,
    ...
    def _vb_terms_bpd(
        ...
        kl = mean_flat(kl) / np.log(2.0) # 对于loss取平均再除log2
