...
def load_data_text(
    *, data_dir, batch_size, image_size, class_cond=False, deterministic=False, data_args=None, 
    ...
    elif task_mode == 'BART_diffusion':

        training_data, model = get_corpus_rocstory(data_args, model, image_size,
                                                padding_mode=padding_mode,split=split,
                                                load_vocab=load_vocab)
        print('return training data. ')

    if data_args.modality in ['roc-aug', 'roc', 'book', 'yelp', 'commonGen', 'commonGen-aug','BART_diffusion'] and data_args.cache_mode=='no':
...
def create_data(data, tokenizer, max_len, term='train'):

    ret, flag = [], True
    for src, ref in tqdm(data,total=len(data)):


        source_ids = tokenizer.encode(src, max_length=max_len, truncation='only_first')
        if flag:
            flag = False
            print("source:", src, len(src), max_len)
            print("reference:", ref, len(ref), max_len)

        if term == 'train':
            reference_ids = tokenizer.encode(ref, max_length=max_len, truncation='only_first')
            features = {'input_ids': source_ids,
                        'decoder_input_ids': reference_ids,
                        'attention_mask': [1] * len(source_ids),
                        'decoder_attention_mask': [1] * len(reference_ids)
                        }
        elif term =='dev':
            reference_ids = tokenizer.encode(ref, max_length=max_len, truncation='only_first')
...
def load_data_control(args,data_dir,tokenizer,pretrain_model,term,device):
    print('loading data control douban movie comments')
    tokenizer = tokenizer
    pretrain_model = pretrain_model
    print(f'loading from {data_dir}')
    data = load_data(data_dir)
    data = create_data(data, tokenizer, max_len=150, term=term)
    data = KeyDataset(data)
    data = DataLoader(data, batch_size=1, shuffle=True, collate_fn=default_collate)
...
# def helper_tokenize_encode(sentence_lst, vocab_dict, model, seqlen, data_args, padding_mode, prompt_lst):
#     result_train_lst = []
#     group_lst = defaultdict(list)
#     with torch.no_grad():
#         for input_ids, input_prompt in zip(sentence_lst,prompt_lst):
#
#             tokenized_ = [vocab_dict.get(x, vocab_dict['UNK']) for x in input_ids]
#             prompt = [vocab_dict.get(x, vocab_dict['UNK']) for x in input_prompt]
#             input_ids = prompt + [0] + tokenized_ + [1]
#             group_lst['word_ids'].append(input_ids)
#         print(group_lst['word_ids'][:2])
#
#         if padding_mode == 'block':
...
def optimize_concatenation(group_lst, seqlen):
    concatenated_examples = {}
    total_length = 0
    block_size = seqlen

    for key, values in tqdm(group_lst.items(), total=len(group_lst)):
        concatenated_examples[key] = []
        for input_ids in values:
            concatenated_examples[key].extend(input_ids)

        total_length = len(concatenated_examples[key])
        total_length = (total_length // block_size) * block_size

        # Split by chunks of block_size.
        group_lst[key] = [concatenated_examples[key][i: i + block_size] for i in range(0, total_length, block_size)]

    return group_lst

def helper_tokenize_encode(sentence_lst, vocab_dict, model, seqlen, data_args, padding_mode, split):
    result_train_lst = []
    group_lst = defaultdict(list)
    unk_count = 0  # 初始化计数器
    with torch.no_grad():
        for input_ids in sentence_lst:
            ...
            group_lst['word_ids'].append(input_ids)
        print(group_lst['word_ids'][:2])
        print('unk count is ', unk_count)

        if padding_mode == 'block':
            print('padding mode is block')

            group_lst = optimize_concatenation(group_lst, seqlen)
            # concatenated_examples = {k: sum(group_lst[k], []) for k in tqdm(group_lst.keys(), total=len(group_lst)) }
            # total_length = len(concatenated_examples[list(group_lst.keys())[0]])
            # block_size = seqlen
            # total_length = (total_length // block_size) * block_size
            # # Split by chunks of max_len.
        ...
        # for input_ids in tqdm(group_lst['word_ids'],total=len(group_lst['word_ids'])):
        #     if data_args.experiment.startswith('random'):
        #         hidden_state = model(torch.tensor(input_ids))
        #     elif data_args.experiment == 'gpt2_pre_compress':
        #         input_ids2 = torch.tensor(input_ids).to(model.device)
        ...
        input_ids_tensor = torch.tensor(group_lst['word_ids'])

        #

        batch_size = 32
        num_batches = (len(group_lst['word_ids']) + batch_size - 1) // batch_size

        with torch.no_grad():
            for batch_idx in tqdm(range(num_batches), desc='Processing batches'):
                start_idx = batch_idx * batch_size
                end_idx = min(start_idx + batch_size, len(group_lst['word_ids']))
                batch_input_ids = torch.tensor(group_lst['word_ids'][start_idx:end_idx])

                if data_args.experiment.startswith('random'):
                    hidden_state = model(batch_input_ids)
                elif data_args.experiment == 'gpt2_pre_compress':
                    input_ids2 = batch_input_ids.to(model.device)
                    input_embs = model.transformer.wte(input_ids2)
                    hidden_state = model.down_proj(input_embs)
                    hidden_state = hidden_state * data_args.emb_scale_factor
                elif data_args.experiment == 'glove':
                    hidden_state = model(batch_input_ids)
                ...
                # 批量处理的结果添加到列表中
                for input_ids, hs in zip(batch_input_ids, hidden_state):
                    result_train_lst.append({'input_ids': input_ids, 'hidden_states': hs.cpu().tolist()})
    return result_train_lst
...
def get_corpus_movie_comments(data_args, tokenizer, model, image_size, padding_mode='block',
                        split='train', load_vocab=None):
    import csv, torch, json
    if data_args.experiment == 'lm':
        if data_args.modality == 'BART_diffusion':
            print('loading dataset from movie comments')
            tokenizer = tokenizer

            sentence_lst = []
            if split == 'train':
                path = f'/data/hanyc/project/Diffusion-LM-main/datasets/train.csv'
            elif split == 'valid':
                path = f'/data/hanyc/project/Diffusion-LM-main/datasets/valid.csv'
            data = pd.read_csv(path, sep=',', header='infer', encoding='utf-8')
            for index, row in data.iterrows():
                sentences = row['ref']
    ...
    #
    # return {'train': lm_datasets}, model
...
def get_corpus_rocstory(data_args, model, image_size, padding_mode='block',
    ...
    if data_args.experiment_mode == 'lm':
        ...
        if data_args.modality == 'roc-aug':
            ...
            if split in ['train', 'valid', 'test']:
                with open(path, 'r') as ff:
                    for row in ff:
                        ...
                        label_lst = row.split('||')[0]
                        label_lst = label_lst.split('|')
                        label_lst = "".join(label_lst)
                        label_lst = [x.text for x in tokenizer(label_lst)]
                        prompt_lst.append(label_lst)

                        word_lst = [x.text for x in tokenizer(word_lst)]
                        sentence_lst.append(word_lst)
            print('sentence_lst:', sentence_lst[:2])
            print('prompt-lst:', prompt_lst[:2])

        elif data_args.modality == 'BART_diffusion':
            print('loading dataset from movie comments')
            import spacy
            sentence_lst = []
            ...
            if split == 'train':
                path = f'{data_args.data_movie_path}/comment_train.csv'
                temp_file = f'{data_args.checkpoint_path}/train.pkl'
            elif split == 'valid':
                path = f'{data_args.data_movie_path}/comment_valid.csv'
                temp_file = f'{data_args.checkpoint_path}/valid.pkl'

            if os.path.exists(temp_file):
                print('loading from cache')
                sentence_lst = pickle.load(open(temp_file, 'rb'))
            else:
                if split == 'train':
                    data = pd.read_csv(path, sep=',', header='infer', encoding='utf-8')
                if split == 'valid':
                ...
                if split == 'train':
                    train_path = f'{data_args.checkpoint_path}/train.pkl'
                    with open(train_path, 'wb') as f:
                        pickle.dump(sentence_lst, f)
                elif split == 'valid':
                    valid_path = f'{data_args.checkpoint_path}/valid.pkl'
    ...
    if load_vocab is None:
        ...
        if os.path.exists(path_save_vocab):
            print(f'load the vocab from {path_save_vocab}')

        else:

            print(f'save the vocab to {path_save_vocab}')
            with open(path_save_vocab, 'w', encoding="utf8") as f:
    ...
    if data_args.experiment_mode == 'lm' and data_args.modality in ['roc-aug', 'roc', 'yelp', 'commonGen', 'commonGen-aug'] \
        ...
        # data_path = f'{data_args.checkpoint_path}/{split}_data.pkl'
        #
        # if os.path.exists(data_path):
        #     print(f'loading from {data_path}')
        #     result_train_lst = pickle.load(open(data_path, 'rb'))
        # else:

        result_train_lst = helper_tokenize_encode(sentence_lst, vocab_dict, model, image_size ** 2, data_args,
                                                      padding_mode, split)
            # data_path = f'{data_args.checkpoint_path}/{split}_data.pkl'
            # if not os.path.exists(data_path):
            #     print(f'save the train data to {data_path}')
            #     with open(data_path, 'wb') as f:
            #         pickle.dump(result_train_lst, f)
